...
Pi Web также доступен как кроссплатформенное десктоп-приложение на Electron — полный веб-интерфейс плюс нативные удобства:

- **Системный трей**: щёлкните значок в трее, чтобы переключить окно; при закрытии окна Pi Web остаётся работать в трее.
