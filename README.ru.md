...
Десктоп-приложение на базе локального веб-интерфейса для [pi coding agent](https://github.com/badlogic/pi-mono). Pi Coder читает локальные файлы сессий pi и предоставляет просмотр сессий, общение с агентом в реальном времени, настройку моделей, управление навыками и предварительный просмотр файлов проекта.

![Pi Coder показывает одну и ту же сессию pi рядом с CLI: структурированный Markdown, вызовы инструментов и навигация по проекту](https://raw.githubusercontent.com/agegr/pi-web/main/docs/screenshot2.png)

Одна и та же сессия pi в CLI и Pi Coder: структурированные вызовы инструментов, удобочитаемый Markdown, просмотр сессий и наглядные результаты.

## Быстрый старт

Для работы Pi Coder требуется Node.js **22.19.0** или новее. Проверьте версию командой `node --version`.

Быстрый старт (апстрим Pi Web):
...
Затем откройте [http://127.0.0.1:30141](http://127.0.0.1:30141). После запуска сервера CLI попытается автоматически открыть браузер. По умолчанию Pi Coder прослушивает только `127.0.0.1`.
...
Pi Coder может вызывать агента с широкими полномочиями. Basic Auth не шифрует пароль при передаче, поэтому не публикуйте обычный HTTP-сервер в интернете. Для удалённого доступа используйте HTTPS через доверенный обратный прокси или надёжное VPN-подключение.
...
Для серверных запросов к моделям и API Pi Coder использует стандартные переменные окружения `HTTP_PROXY`, `HTTPS_PROXY` и `NO_PROXY`.
...
Pi Coder также доступен как кроссплатформенное десктоп-приложение на Electron — полный веб-интерфейс плюс нативные удобства:

- **Системный трей**: щёлкните значок в трее, чтобы переключить окно; при закрытии окна Pi Coder остаётся работать в трее.
- **Глобальная горячая клавиша** `Ctrl+Shift+P` (на macOS `Cmd+Shift+P`) для показа или скрытия окна откуда угодно.
- **Запуск при входе в систему** (выключено по умолчанию, переключается в меню приложения).
- **Нативные уведомления** по завершении задачи, даже когда окно скрыто в трей.
- **Следует системной теме** (светлая / тёмная).
- **Одиночный экземпляр**: повторный запуск фокусирует уже работающее окно вместо открытия нового.
- **Автообновление внутри приложения** для установочных сборок.

Установочные пакеты распространяются через [GitHub Releases](https://github.com/agegr/pi-web/releases); портативная сборка запускается без установки и не обновляется автоматически. Подробности сборки и упаковки см. в [документе о десктоп-сборке и распространении](./docs/desktop.md).
...
- **Интеграция файлового браузера с системой**
...
- **Свои темы**: с помощью штатного JSON-файла пользовательских тем Pi вы можете задать собственную цветовую тему интерфейса. Примеры лежат в `docs/themes`.

  - Чтобы применить: поместите изменённый JSON-файл в `~/.pi/agent/themes/`.
...
- **Каталог данных**: по умолчанию Pi Coder читает `~/.pi/agent/sessions`. Чтобы указать другой каталог агента pi, задайте `PI_CODING_AGENT_DIR`.
...
- **Git worktree**: условия отображения переключателя, создание новых рабочих деревьев и их удаление описаны в документе [Worktrees in Pi Coder](./docs/worktrees.md).
...
electron/             # опциональная десктоп-оболочка, упаковывающая веб-интерфейс как нативное приложение
  main/               # окно, трей, меню, IPC, глобальная горячая клавиша, автообновление, подпроцесс server
  preload/            # API contextBridge, доступный рендереру
  icons/              # значок в трее и entitlements для macOS
